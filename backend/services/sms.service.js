...
const sendBookingConfirmation = async (phoneNumber, bookingDetails) => {
    const message = `🎉 Congratulations ${bookingDetails.customer_name || ''}!\n` +
                   `Your booking with Bens Trans is successful!\n` +
                   `Ref: ${bookingDetails.reference_number}\n` +
                   `Route: ${bookingDetails.route_from} → ${bookingDetails.route_to}\n` +
                   `Date: ${bookingDetails.travel_date}\n` +
                   `Time: ${bookingDetails.departure_time}\n` +
                   `Seats: ${bookingDetails.seats_booked}\n` +
                   `Total: KSh ${bookingDetails.total_amount}\n` +
                   `Vehicle: ${bookingDetails.vehicle_number}\n` +
                   `Pay by: ${bookingDetails.payment_deadline}\n` +
                   `Thank you for choosing Bens Trans! Safe travels!`;
    
    return await sendSMS(phoneNumber, message);
};

const sendPaymentConfirmation = async (phoneNumber, paymentDetails) => {
    const message = `🎉 Congratulations ${paymentDetails.customer_name || ''}!\n` +
                   `Payment received by Bens Trans!\n` +
                   `Receipt: ${paymentDetails.mpesa_receipt}\n` +
                   `Amount: KSh ${paymentDetails.amount}\n` +
                   `Ref: ${paymentDetails.reference_number}\n` +
                   `Route: ${paymentDetails.route_from || ''} → ${paymentDetails.route_to || ''}\n` +
                   `Date: ${paymentDetails.travel_date || ''}\n` +
                   `Your seat is fully confirmed. Have a safe journey!`;
